...
void WebServer::eventListen()
{
    ...
    assert(ret != -1);
    utils.setnonblocking(m_pipefd[1]);                     // 主线程端设置为非阻塞 (对主线程来说是写，对工作线程来说是读)
    utils.addfd(m_epollfd, m_pipefd[0], false, 0);         // 监听工作线程端 (对工作线程来说是写，对主线程来说是读)
...
bool WebServer::dealclinetdata()
{
    ...
    if (0 == m_LISTENTrigmode)
    {
        ...
        timer(connfd, client_address);   // 接受到新连接，则为之创建对应的定时器
    }
    // 监听套接字为ET模式
    else
    {
